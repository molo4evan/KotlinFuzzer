...
class LiteralFactory(private val resultType: Type): Factory<Literal>() {
    override fun produce(): Literal {
        ...
        when(resultType){
            ...
            TypeList.LONG -> literal = Literal((PseudoRandom.random() * Long.MAX_VALUE).toLong(), resultType)
            TypeList.FLOAT -> {
                val value = String.format("%.${ProductionParams.floatingPointPrecision?.value() ?: 0}EF", ((PseudoRandom.random() * (Float.MAX_VALUE - Float.MIN_VALUE)) + Float.MIN_VALUE).toFloat()).toFloat()
                literal = Literal(value, resultType)
            }
            TypeList.DOUBLE -> {
                val value = String.format("%.${ProductionParams.floatingPointPrecision?.value() ?: 0}E", ((PseudoRandom.random() * (Double.MAX_VALUE - Double.MIN_VALUE)) + Double.MIN_VALUE)).toDouble()
                literal = Literal(value, resultType)
            }
            TypeList.STRING -> {
                ...
                val str = charArrayOf()
                for (i in 0 until size) str[i] = ((('z' - 'a') * PseudoRandom.random()).toInt() + 'a'.toInt()).toChar()
